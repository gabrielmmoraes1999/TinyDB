...
// Shave off trailing zeros and decimal point, if possible.
     ...
     * double, the {@link BigDecimal#BigDecimal(double)} constructor will be
     ...
     * create one with a value of 1 (Integer). If there is such a property, and
     * if it is an Integer, Long, Double, Float, BigInteger, or BigDecimal then
     * add one to it. No overflow bounds checking is performed, so callers
     * should initialize the key prior to this call with an appropriate type
     ...
     * {@link Float} values will be converted exactly. When <code>false</code>,
     * they will be converted to {@link String} values before converting to
     * {@link BigDecimal}.
     * @return BigDecimal conversion of the original value, or the defaultValue
     * if unable to convert.
     */
     ...
     * method or one of its super class definitions
     */
     ...
     *
     * If either of the numbers are Double or Float instances, then they are
     * checked to have a finite value. If either value is not finite (NaN or
     * &#177;infinity), then this function will always return false. If both
     * numbers are finite, they are first checked to be the same type and
     * implement {@link Comparable}. If they do, then the actual
     ...
     * <p>
     * If
     * <pre>{@code indentFactor > 0}</pre> and the {@link JSONObject} has only
     * one key, then the object will be output on a single line:
     * <pre>{@code {"key": 1}}</pre>
     *
     * <p>
